...
- 関連者行の **「相関図に追加」** は、当該関連者が DB 上 **主体者として関係保存を実行済み**（`has_relations` / `executed_as_master` 相当）のときだけ **「関連者を探す」**（❸ 表内・当該関連者を主体に切り替える導線）の左に表示され、クリックで相関図タブへ移動してその人物を中心候補に追加する。
